# Graph Report - agente-personal  (2026-07-14)

## Corpus Check
- 53 files · ~14,173 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 336 nodes · 365 edges · 40 communities (36 shown, 4 thin omitted)
...
1. `ErrorAgente` - 37 edges
2. `Own Agent` - 12 edges
3. `Catálogo de Errores de Desarrollo` - 11 edges
4. `Troubleshooting` - 10 edges
5. `Definición de Módulos` - 10 edges
6. `Comandos Disponibles (Fase 1)` - 8 edges
7. `Agente Personal de Automatización` - 8 edges
8. `Catálogo de errores` - 8 edges
9. `Catálogo de Comandos` - 8 edges
10. `Catálogo de primitivas` - 8 edges

## Surprising Connections (you probably didn't know these)
- `loop()` --indirect_call--> `ErrorAgente`  [INFERRED]
  main.py → src/models/error.py
- `clasificar()` --references--> `ErrorAgente`  [EXTRACTED]
  src/interpreter/classifier.py → src/models/error.py
- `ejecutar()` --references--> `ErrorAgente`  [EXTRACTED]
  src/executor/executor.py → src/models/error.py
- `_ejecutar_accion()` --references--> `ErrorAgente`  [EXTRACTED]
  src/executor/executor.py → src/models/error.py
- `subir_volumen()` --references--> `ErrorAgente`  [EXTRACTED]
  src/executor/functions.py → src/models/error.py
...
Cohesion: 0.12
Nodes (18): Dispatcher que mapea nombres de funciones internas a sus implementaciones.  Cent, ejecutar(), _ejecutar_accion(), Itera las acciones de una Intencion y las ejecuta secuencialmente.  Aplica la po, Itera las acciones de la Intencion. Detiene en el primer error (fail-fast)., Ejecuta una acción individual según su tipo., construir(), Constructor de objetos ``Intencion``.  Transforma los tokens ya clasificados en (+10 more)

### Community 1 - "ErrorAgente"
Cohesion: 0.08
Nodes (36): ajustar_brillo(), ajustar_volumen(), bajar_volumen(), cerrar_proceso(), consultar_sistema(), consultar_web(), crear_archivo(), _disparar_notificacion() (+28 more)
...
Cohesion: 0.15
Nodes (9): Catálogo de verbos reconocidos y sus mapeos a primitivas.  Separado de ``config., clasificar(), Clasificador de comandos.  Determina si un comando es una **primitiva** (verbo r, dividir(), Tokenizador del lenguaje de comandos.  Convierte el texto crudo del usuario en u, Divide el texto en tokens usando ``shlex.split()``.      Respeta comillas simple, interpretar(), Interpretación de lenguaje natural vía Ollama.  Fallback para cuando el classifi (+1 more)
...
- **Why does `ErrorAgente` connect `ErrorAgente` to `models.py`, `interpreter.py`, `main.py`?**
